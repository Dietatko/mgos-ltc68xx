...
bool read_update_write_reg_diff(
   struct mgos_ltc68xx1* handle, 
   struct mgos_ltc68xx_data* data, 
   uint16_t readReg, 
   uint16_t writeReg, 
   void (*updateFunc)(struct mgos_ltc68xx_data*))
{
...
bool read_update_write_reg_same(struct mgos_ltc68xx1* handle, struct mgos_ltc68xx_data* data, uint16_t readReg, uint16_t writeReg, void (*updateFunc)(int, uint8_t*))
{
   return read_update_write_reg_diff(handle, data, readReg, writeReg, 
      LAMBDA(void _(struct mgos_ltc68xx_data* d) {
         for(int i = d->chainLength - 1; i >= 0; i--)
            updateFunc(i, mgos_ltc68xx_get_chip_data(d, i));
      }));
}

bool read_update_write_config_reg(struct mgos_ltc68xx1* handle, struct mgos_ltc68xx_data* data, void (*updateFunc)(int, uint8_t*))
{
   return read_update_write_reg_same(handle, data, READ_CONFIG_REGISTER, WRITE_CONFIG_REGISTER, updateFunc);
}

bool setup_config_reg(struct mgos_ltc68xx1* handle, struct mgos_ltc68xx_data* data)
{
   return read_update_write_config_reg(handle, data, LAMBDA(void _(DO_NOT_WARN_UNUSED int i, uint8_t* chipData) {
      // Start reference
      SET_BIT(chipData[0], 2);

      // Set ADC Mode option
      CLEAR_BIT(chipData[0], 0);
      chipData[0] |= GET_ADC_OPTION(handle->adcMode);
   }));
}
